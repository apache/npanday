...
using NMaven.ProjectImporter.Digest.Algorithms;

using NMaven.ProjectImporter.Parser.VisualStudioProjectTypes;

/// Author: Leopoldo Lee Agdeppa III

namespace NMaven.ProjectImporter.Digest
{
                ...
                foreach (ProjectReference projectReference in projectDigest.ProjectReferences)
                {
                    if (!projDigestDictionary.ContainsKey(projectReference.Name))
                    {
                        ...
                        string errMsg = string.Format(
                            ...
                            + "\nWould you like to include \"{1}\" Generating NMaven Project Poms?"
                        ...
                        else
                        {
                            MessageBox.Show(
                                string.Format(
                                "Please Make sure that Artifact[GroupId: {0}, ArtifactId: {0}] exists in your NMaven Repository, " +
                                "\n Or an error will occur during NMaven-Build due to Missing Artifact Dependency!!!",
                                prjRefDigest.AssemblyName),
