...
namespace NMaven.Utils
{
    public class NMavenPomHelperUtility
    {

        private FileInfo pom;

        public NMavenPomHelperUtility(Solution solution, Project project)
        {
        ...
        public NMavenPomHelperUtility(string pom)
        {
            this.pom = new FileInfo(pom);
        }

        public NMavenPomHelperUtility(FileInfo pom)
        {
            this.pom = pom;
        }
        ...
        private DirectoryInfo GetBuildSrcDir(string src, bool testSource)
        {
            ...
            if (string.IsNullOrEmpty(src) && "csharp".Equals(NMavenCompilerPluginLanguage, StringComparison.OrdinalIgnoreCase))
            {
                src = defaultDirs[0];
            }


            if (string.IsNullOrEmpty(src) && "vb".Equals(NMavenCompilerPluginLanguage, StringComparison.OrdinalIgnoreCase))
            {
                src = defaultDirs[1];
            }
        ...
        public string NMavenCompilerPluginLanguage
        {
            get
            {

                string lang = GetNMavenCompilerPluginConfigurationValue("language");
        ...
        public string CompilerPluginConfigurationKeyfile
        {
            get
            {
                string key = GetNMavenCompilerPluginConfigurationValue("keyfile");
            ...
            set
            {
                ...
                {
                    SetNMavenCompilerPluginConfigurationValue("keyfile", null);
                    return;
                }
                ...
                SetNMavenCompilerPluginConfigurationValue("keyfile", str);
            }
        }

        public string GetNMavenCompilerPluginConfigurationValue(string config)
        {
        ...
        public NMaven.Model.Pom.Model ReadPomAsModel()
        {
            return NMavenPomHelperUtility.ReadPomAsModel(this.pom);
        }

        public static NMaven.Model.Pom.Model ReadPomAsModel(FileInfo pomfile)
        {
            if (!pomfile.Exists)
            {
        ...
        public void WriteModelToPom(NMaven.Model.Pom.Model model)
        {
            NMavenPomHelperUtility.WriteModelToPom(this.pom, model);
        }
        ...
        {
            NMaven.Model.Pom.Model model = ReadPomAsModel();
            List<NMaven.Model.Pom.Plugin> plugins = new List<NMaven.Model.Pom.Plugin>();
        ...
        public static void WriteModelToPom(FileInfo pomFile, NMaven.Model.Pom.Model model)
        {
        ...
        public void SetNMavenCompilerPluginConfigurationValue(string config, string value)
        {
            ...
            if (model.build == null)
            {
                model.build = new NMaven.Model.Pom.Build();
            }

            NMaven.Model.Pom.Plugin compilePlugin = null;
            List<NMaven.Model.Pom.Plugin> plugins = new List<NMaven.Model.Pom.Plugin>();

            if (model.build.plugins != null)
            {
                foreach (NMaven.Model.Pom.Plugin plugin in model.build.plugins)
                {
            ...
            {
                compilePlugin = new NMaven.Model.Pom.Plugin();
            ...
            {
                compilePlugin.configuration = new NMaven.Model.Pom.PluginConfiguration();
            }
        ...
        {
            return NMavenPomHelperUtility.GetRelativePath(pom.Directory, file);
        }

        public string GetRelativePathFromPom(DirectoryInfo dir)
        {
            return NMavenPomHelperUtility.GetRelativePath(pom.Directory, dir);
        }

        public string GetRelativePathFromPom(string filename)
        {
            return NMavenPomHelperUtility.GetRelativePath(pom.Directory, new FileInfo(filename));
        }




        public bool IsPomDependency(NMaven.Model.Pom.Dependency dependency)
        {
        ...
        public void RemovePomDependency(NMaven.Model.Pom.Dependency dependency)
        {
        ...
        {
            NMaven.Model.Pom.Dependency dependency = new NMaven.Model.Pom.Dependency(); ;
        ...
        public void AddPomDependency(NMaven.Model.Pom.Dependency dependency)
        {
        ...
        Plugin addPlugin(NMaven.Model.Pom.Model model, string groupId, string artifactId, string version, bool extensions)
        {
            List<NMaven.Model.Pom.Plugin> plugins = new List<NMaven.Model.Pom.Plugin>();
            if (model.build.plugins != null)
            {
            ...
            NMaven.Model.Pom.Plugin plugin = new NMaven.Model.Pom.Plugin();
