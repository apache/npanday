...
namespace NMaven.Utils
{
    public class MavenRunner
    {
           ...
           // _applicationObject is from the main class
            try
            {
                ...
                for (int i = 1; i <= panes.Count; i++)
                {
                    ...
                    if (outputPane.Name == "NMaven Execution Output:")
                        return outputPane;
                }

                OutputWindowPane output = outputWindow.OutputWindowPanes.Add("NMaven Execution Output:");
                return output;
            }
            catch (Exception e)
            {
        ...
        public void stop()
        {
            ...
            {
                stopCalled = true;
                output.OutputString("\nStopping current NMaven process.");
                if (outputThreadEvent != null)
                {
                    outputThreadEvent.WaitOne();
                }
        ...
        private void mvn_process_exited(object sender, System.EventArgs e)
        {
            ...
            if (stopCalled)
            {
                output.OutputString("\nNMaven execution stopped successfully.");
                return;
            }
